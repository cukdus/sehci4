    ...
    <div class="container-fluid">
      <div class="card card-warning card-outline">
        <div class="card-body">
          ...
          <?php endif; ?>
          <form id="formEditAnggota" action="/admin/anggota/update" method="post" enctype="multipart/form-data" novalidate>
            <?= csrf_field() ?>
            ...
            <div class="row g-3">
              ...
              <div class="col-md-4">
                ...
                <select name="jenis_anggota" class="form-select">
                  <option value="aktif" <?= ($anggota['jenis_anggota'] ?? '') === 'aktif' ? 'selected' : '' ?>>Aktif</option>
                  <option value="pasif" <?= ($anggota['jenis_anggota'] ?? '') === 'pasif' ? 'selected' : '' ?>>Pasif</option>
                </select>
              </div>
              ...
              <div class="col-md-12">
                ...
                <?php
                  $rawSkills = (string) ($anggota['basic_skill'] ?? '');
                  $existingSkills = [];
                  if ($rawSkills !== '') {
                      if ($rawSkills[0] === '[') {
...
<script>
  (function(){
    ...
    form.addEventListener('submit', function(e){
      ...
      const existing = document.getElementById('editAnggotaAlert');
      if (existing) { existing.remove(); }
      if (missing.length > 0) {
        e.preventDefault();
        const alertDiv = document.createElement('div');
        alertDiv.className = 'alert alert-danger';
        alertDiv.id = 'editAnggotaAlert';
