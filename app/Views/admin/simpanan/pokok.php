<main class="app-main">
  <div class="container-fluid py-3">
    <div class="d-flex justify-content-between align-items-center">
      ...
      <div>
        <button type="button" class="btn btn-primary btn-sm me-2" id="btnBulkAddPokok">
          <i class="bi bi-plus-lg"></i> Tambah Pokok (Semua)
        </button>
        <button type="button" class="btn btn-success btn-sm" id="btnAddPokokByAnggota">
          <i class="bi bi-person-plus"></i> Tambah Pokok (Per Anggota)
        </button>
      </div>
    </div>
  </div>
...
<div class="modal fade" id="bulkPokokModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Tambah Simpanan Pokok untuk Semua Anggota Aktif</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="bulkPokokForm">
        <div class="modal-body">
          <div id="bulkPokokAlert"></div>
          <div class="alert alert-info">
            Sistem akan membuat data simpanan pokok untuk semua anggota aktif yang memiliki nomor anggota. Jika untuk tanggal yang sama sudah ada, data akan dilewati atau ditimpa sesuai pilihan.
          </div>
          <div class="row g-3">
            <div class="col-md-4">
            ...
            <div class="col-md-4">
              ...
              <small class="text-muted">Default diambil dari setting fee pokok.</small>
            </div>
            <div class="col-md-4">
        ...
        <div class="modal-footer">
          ...
          <button type="submit" class="btn btn-primary" id="bulkPokokSubmit">Proses</button>
        </div>
      </form>
    </div>
  </div>
...
<div class="modal fade" id="singlePokokModal" tabindex="-1" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title">Tambah Simpanan Pokok per Anggota</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <form id="singlePokokForm">
        <div class="modal-body">
          <div id="singlePokokAlert"></div>
          <div class="row g-3">
        ...
        <div class="modal-footer">
          <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Batal</button>
          <button type="submit" class="btn btn-success" id="singlePokokSubmit">Tambah</button>
        </div>
      </form>
    </div>
  </div>
...
<script>
  document.addEventListener('DOMContentLoaded', function(){
    ...
    const btnBulkAddPokok = document.getElementById('btnBulkAddPokok');
    const btnAddPokokByAnggota = document.getElementById('btnAddPokokByAnggota');
    const bulkModal = new bootstrap.Modal(document.getElementById('bulkPokokModal'));
    const singleModal = new bootstrap.Modal(document.getElementById('singlePokokModal'));
    const bulkForm = document.getElementById('bulkPokokForm');
    const bulkAlert = document.getElementById('bulkPokokAlert');
    const bulkSubmit = document.getElementById('bulkPokokSubmit');
    const bulkTanggalSimpan = document.getElementById('bulkTanggalSimpan');
    const bulkJumlah = document.getElementById('bulkJumlah');
    const singleForm = document.getElementById('singlePokokForm');
    const singleAlert = document.getElementById('singlePokokAlert');
    const singleSubmit = document.getElementById('singlePokokSubmit');
    ...
    function loadFeePokok(){
      fetch('/admin/api/simpanan/config')
        .then(x=>x.json())
        .then((j)=>{
          const fee = parseFloat(j.fee_pokok || 0) || 0;
          if (fee > 0) {
    ...
    btnBulkAddPokok.addEventListener('click', function(){
      bulkAlert.innerHTML = '';
      bulkForm.reset();
    ...
    btnAddPokokByAnggota.addEventListener('click', function(){
    ...
    bulkForm.addEventListener('submit', function(e){
      ...
      fetch('/admin/api/simpanan/pokok/bulk-add', { method:'POST', body: fd })
        .then(async (x)=>{ const j = await x.json(); if(!x.ok) throw j; return j; })
        .then((j)=>{
          const lines = [];
          lines.push('Selesai membuat data simpanan pokok.');
    ...
    singleForm.addEventListener('submit', function(e){
      ...
      fetch('/admin/api/simpanan/pokok/add-by-anggota', { method:'POST', body: fd })
        .then(async (x)=>{ const j = await x.json(); if(!x.ok) throw j; return j; })
        .then(()=>{
          singleAlert.innerHTML = '<div class="alert alert-success mb-0">Simpanan pokok berhasil ditambahkan.</div>';
          page = 1;
          load();
        })
        .catch((err)=>{
