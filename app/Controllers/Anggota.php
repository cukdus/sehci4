...
class Anggota extends Controller
{
    ...
    private function processBase64ProfilePhoto(string $fotoBase64, string $uploadDir): string
    {
        ...
        $extension = self::PROFILE_PHOTO_EXTENSIONS[$mime] ?? 'jpg';
        $finalName = 'foto_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $extension;
        $finalPath = $uploadDir . DIRECTORY_SEPARATOR . $finalName;

        if (file_put_contents($finalPath, $binary) === false) {
            @unlink($finalPath);
            throw new \RuntimeException('File foto gagal disimpan');
        }
    ...
    private function processUploadedProfilePhoto($fotoFile, string $uploadDir): string
    {
        ...
        $extension = self::PROFILE_PHOTO_EXTENSIONS[$mime] ?? strtolower((string) ($fotoFile->getClientExtension() ?: $fotoFile->guessExtension() ?: 'jpg'));
        $finalName = 'foto_' . time() . '_' . bin2hex(random_bytes(4)) . '.' . $extension;

        try {
            $fotoFile->move($uploadDir, $finalName);
        } catch (\Throwable $e) {
            throw new \RuntimeException('File foto gagal dipindahkan');
        }
    ...
    public function apiSimpananSummary()
    {
        ...
        try {
            ...
            if (in_array('pinjaman', $tables, true)) {
                $sumPinjaman = (float) ($db->table('pinjaman')->selectSum('jumlah_pinjaman')->where('id_anggota', $idAnggota)->where('status', 'aktif')->get()->getRowArray()['jumlah_pinjaman'] ?? 0);
                $pinjamanRows = $db
                    ->table('pinjaman')
        ...
        foreach ($chartKeys as $key) {
            $pokok = (float) ($chartMap[$key]['pokok'] ?? 0);
            $wajib = (float) ($chartMap[$key]['wajib'] ?? 0);
            $sukarela = (float) ($chartMap[$key]['sukarela'] ?? 0);
            $pinjaman = (float) ($chartMap[$key]['pinjaman'] ?? 0);
            $chart['categories'][] = $formatBucketLabel($key);
            $chart['series']['pokok'][] = $pokok;
            $chart['series']['wajib'][] = $wajib;
            $chart['series']['sukarela'][] = $sukarela;
            $chart['series']['pinjaman'][] = $pinjaman;
            $chart['series']['neraca'][] = ($pokok + $wajib + $sukarela) - $pinjaman;
        }
